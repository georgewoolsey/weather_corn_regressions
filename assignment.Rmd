...
<span style="color: blue;">Using a quadratic time trend (i.e., year + year^2) shows a significant time trend in the corn yields in  Winneshiek County, Iowa from `r as.character(min(winneshiek_yields$year_id))` to `r as.character(max(winneshiek_yields$year_id))`. Based on a the quadratic model, there is a slightly *increasing* rate of corn yield over time. The quadratic time trend explains **`r scales::percent(summary(lm_yield_year_quad)$r.squared)`** of the variation in corn yields in  Winneshiek County, Iowa. Using the `anova()` function to compare the quadratic model with the simple linear model allows on to test which model provides the best parsimonious fit of the data. Although the quadratic model is significant, the ANOVA test has a resulting p-value of **`r round(anova_m1_m2[2, "Pr(>F)"], 3)`**. This result means that adding the quadratic term did not significantly improve the model over the simple linear model.</span>
...
<span style="color: blue;">A simple linear regression of corn yield on mean summer maximum temperature shows that temperature alone explains **`r scales::percent(summary(lm_yield_tmax)$r.squared)`** of the variation in corn yields in  Winneshiek County, Iowa. This model performs worse than the simple time trend of annual corn yield investigated above. A multiple linear regression of corn yield on year and mean summer maximum temperature explains **`r scales::percent(summary(lm_yield_year_tmax)$r.squared)`** of the variation in corn yields in  Winneshiek County, Iowa. Using the `anova()` function to compare the model including year and temperature with the simple linear model including only year allows on to test which model provides the best parsimonious fit of the data. The ANOVA test has a resulting p-value of **`r round(anova_m1_m4[2, "Pr(>F)"], 3)`**. This result means that adding the mean summer maximum temperature did not significantly improve the model over the simple linear model on year alone.</span>
